...
router.get("/:listId/stats", authorizeListRole("Member"), async (req, res) => {
  const { listId } = req.params;
  ...
  try {
    ...
    const solved = shoppingList.items.filter(item => item.purchased).length;
    const unsolved = shoppingList.items.filter(item => !item.purchased).length;
...
// Získání statistik pro všechny seznamy uživatele
router.get("/stats/overview", async (req, res) => {
  ...
  try {
    ...
    const overview = shoppingLists.map(list => ({
      title: list.title,
      totalItems: list.items.length,
    }));

    res.status(200).json({ status: "success", overview });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
